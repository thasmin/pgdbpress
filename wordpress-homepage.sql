...
SELECT option_value FROM wp_options WHERE option_name = '_transient_timeout_is_multi_author' LIMIT 1
SELECT option_value FROM wp_options WHERE option_name = '_transient_is_multi_author' LIMIT 1
SELECT DISTINCT post_author FROM wp_posts WHERE post_type = 'post' AND post_status = 'publish' LIMIT 2
INSERT INTO `wp_options` (`option_name`, `option_value`, `autoload`) VALUES ('_transient_is_multi_author', '0', 'yes') ON DUPLICATE KEY UPDATE `option_name` = VALUES(`option_name`), `option_value` = VALUES(`option_value`), `autoload` = VALUES(`autoload`)
